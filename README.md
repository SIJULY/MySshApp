...
将 sync_server.py 文件上传到你 VPS 的 ~/sync_server 目录中。

(极其重要!) 编辑 sync_server.py 文件，修改 JWT_SECRET_KEY：
...
在 ~/sync_server 目录中，运行脚本：

python3 sync_server.py


服务器会启动并显示类似 * Running on http://127.0.0.1:5000 的信息。

现在，按 Ctrl+C 停止它。

输入 ls，你会看到一个新文件 sync.db 已经被创建。数据库已准备就绪。
...
如果你使用云服务商 (阿里云/腾讯云/AWS等):
请登录到你的云服务商网页控制台，找到“安全组”或“防火墙”规则，为这台 VPS 添加入站规则，允许 TCP 流量访问 5000 端口。
...
pyinstaller --noconsole --name MySshApp ssh_client.py
...
注意: venv\Lib... 的路径取决于你的 Python 环境。
